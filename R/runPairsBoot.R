...
runPairsBoot <- function(eList, pairResults, 
                         ...
                         blockLength = 200, yr_start, yr_end, con, db_table) {
  ...
  for (iBoot in 1:(2*nBoot)){
    ...
    if (!inherits(possibleError3, "error") & 
      ...
      #Total Change delta*
      cDeltaTotal <- (2 * regDeltaConc) - (c22 - c11)
      fDeltaTotal <- (2 * regDeltaFlux) - (f22 - f11)
      
      #CQTC Change delta*
      regfRSpart <- (pairResults$x20[2] - pairResults$x10[2]) 
      regcRSpart <- (pairResults$x20[1] - pairResults$x10[1]) 
      cRSpart <- (2 * regcRSpart) - (c20 - c10)
      fRSpart <- (2 * regfRSpart) - (f20 - f10)
      
      #QTC Change delta*
      regcFDpart <- regDeltaConc - regcRSpart
      regfFDpart <- regDeltaFlux - regfRSpart
      cFDpart <- (2 * regcFDpart) - (c22 - c11 - c20 + c10)
      fFDpart <- (2 * regfFDpart) - (f22 - f11 - f20 + f10)
      
      #ouput to database
      results <- as.data.frame(matrix(ncol = 13, nrow = 1))
      colnames(results) <- c("site_no","parameter_cd","yr_start","yr_end","model_start","model_end",
                             "c_tc","c_cqtc","c_qtc","f_tc","f_cqtc","f_qtc","iboot")
      site_no <- eList$INFO$site_no
      parameter_cd <- eList$INFO$paramNumber
      results[1, ] <- c(site_no, parameter_cd, yr_start, yr_end, year1, year2, cDeltaTotal, cRSpart, cFDpart,
                        fDeltaTotal, fRSpart, fFDpart, iBoot + startSeed)
      
      dbWriteTable(con,  db_table, value=results, append=TRUE, row.names=FALSE)
      # END XXX
